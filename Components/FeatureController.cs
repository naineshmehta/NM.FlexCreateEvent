...
namespace NM.Modules.NM.FlexCreateEvent.Components
{
    /// -----------------------------------------------------------------------------
    /// <summary>
    /// The Controller class for NM.FlexCreateEvent
    /// 
    ...
    public class FeatureController : ModuleSearchBase, IPortable, IUpgradeable
    {
        ...
        public override IList<SearchDocument> GetModifiedSearchDocuments(ModuleInfo moduleInfo, DateTime beginDate)
        {
            ...
            foreach (var item in items)
            {
                ...
                var content = string.Format("{0}<br />{1}", item.Title, item.ShortDescription);

                var searchDocumnet = new SearchDocument
                {
                    UniqueKey = string.Format("Items:{0}:{1}", moduleInfo.ModuleID, item.EventId),  // any unique identifier to be able to query for your individual record
        ...
        public string ExportModule(int moduleId)
        {
            ...
            var itemList = items as IList<Event> ?? items.ToList();

            if (!itemList.Any()) return string.Empty;

            sb.Append("<Items>");

            foreach (Event item in itemList)
            {
                sb.Append("<Event>");
                ...
                sb.AppendFormat("<EventId>{0}</EventId>", item.EventId);
                sb.AppendFormat("<ShortDescription>{0}</ShortDescription>", XmlUtils.XMLEncode(item.ShortDescription));
                sb.AppendFormat("<Title>{0}</Title>", XmlUtils.XMLEncode(item.Title));
        ...
        public void ImportModule(int moduleId, string content, string version, int userId)
        {
            ...
            var xmlNodeList = items.SelectNodes("Event");
            ...
            foreach (XmlNode item in xmlNodeList)
            {
                ...
                newItem.ShortDescription = item.SelectSingleNode("ShortDescription").InnerText;
